...
# Tests for simTrialData() — pure R, no ASReml needed.
# G = "auto" (default) generates a random SPD covariance matrix.
...
test_that("MET-only: total plots = nvar * nsite * nrep", {
  ...
  out <- simTrialData(nvar = nvar, nsite = nsite, nrep = nrep,
...
test_that("Treatment factor levels match 'treatments' argument", {
  trt <- c("Dry", "Irr", "Wet")
  out <- simTrialData(nvar = 6L, nsite = 3L, treatments = trt, nrep = 2L, seed = 11L, verbose = FALSE)
  expect_equal(levels(out$data$Treatment), trt)
})

test_that("Rep factor has nrep levels", {
  out <- simTrialData(nvar = 4L, nsite = 3L, nrep = 3L,
...
test_that("same seed produces identical data", {
  out1 <- simTrialData(nvar = 4L, nsite = 3L, nrep = 2L,
                       seed = 42L, verbose = FALSE)
  out2 <- simTrialData(nvar = 4L, nsite = 3L, nrep = 2L,
                       seed = 42L, verbose = FALSE)
...
test_that("MET-only: G is nsite x nsite", {
  nsite <- 4L
  out <- simTrialData(nvar = 4L, nsite = nsite, nrep = 2L,
...
test_that("G is symmetric positive definite", {
  out <- simTrialData(nvar = 6L, nsite = 4L, nrep = 2L,
...
test_that("outfile writes CSV", {
  ...
  on.exit(unlink(tmp))
  simTrialData(nvar = 4L, nsite = 3L, nrep = 2L,
...
test_that("prime nvar produces rows_per_block = 1 layout", {
  ...
  out <- simTrialData(nvar = 7L, nsite = 3L, nrep = 2L,
...
test_that("Row values within expected range per site (balanced)", {
  nvar <- 4L; nrep <- 2L
  out <- simTrialData(nvar = nvar, nsite = 3L, nrep = nrep,
                      incidence = "balanced",
...
test_that("balanced incidence: all nvar varieties in every site", {
  nvar <- 6L; nsite <- 3L
  out <- simTrialData(nvar = nvar, nsite = nsite, nrep = 2L,
...
test_that("unbalanced: total plots < nvar * nsite * nrep", {
  out <- simTrialData(nvar = 20L, nsite = 6L, nrep = 2L,
...
test_that("unbalanced: incidence matrix stored in params", {
  out <- simTrialData(nvar = 10L, nsite = 4L, nrep = 2L,
...
test_that("unbalanced: plot count matches incidence colSums * nrep", {
  out <- simTrialData(nvar = 15L, nsite = 5L, nrep = 2L,
...
test_that("user matrix: correct plot count", {
  ...
  out <- simTrialData(nvar = 8L, nsite = 4L, nrep = 2L,
...
test_that("user logical matrix accepted", {
  ...
  inc[1, 1] <- FALSE
  expect_no_error(
    simTrialData(nvar = 6L, nsite = 3L, nrep = 2L,
...
test_that("reg_max_sites_pct=1.0 keeps all varieties fully connected", {
  ...
  out <- simTrialData(nvar = 10L, nsite = 5L, nrep = 2L,
                      incidence   = "unbalanced",
...
test_that("invalid core_pct errors informatively", {
  expect_error(
    simTrialData(nvar = 10L, nsite = 4L,
                 incidence   = "unbalanced",
